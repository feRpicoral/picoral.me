...
tagline: >-
  ...
  vez de pagar una API comercial?" con una metodología reproducible.
summary: >-
  Un artefacto de ingeniería enfocado que compara Llama 3.1 8B autoalojado,
  cuantizado con AWQ-INT4 en vLLM, contra APIs comerciales en throughput,
  TTFT/TPOT bajo concurrencia, calidad de cuantización (MMLU/GSM8K/HellaSwag) y
  $/1M tokens. La config de serving, el harness de benchmark, el wrapper de
  eval, el modelo de costos y el pipeline de gráficos están conectados y
  ensayados localmente en M1; los números reales llegan tras la corrida en
  RunPod RTX A5000.
...
faq:
  ...
  - q: ¿Cómo se mantiene la metodología defendible?
    a: >-
      El hardware (RunPod RTX A5000 24 GB a $0.27/hr), los SHAs del modelo
      (baseline BF16 de meta-llama/Llama-3.1-8B-Instruct y la variante AWQ-INT4
      de hugging-quants), la versión de vLLM, la carga de trabajo (traza
      ShareGPT con concurrencia 1/4/16/32/64, 256 prompts por nivel) y las
      fechas de las fuentes de precios están todos commiteados en
      docs/methodology.md y en el JSON de resultados. El pipeline completo se
      ensaya localmente en un MacBook M1 base contra un modelo pequeño (Qwen 2.5
      0.5B) antes de gastar cualquier minuto de GPU pago, así el harness se
      depura en hardware gratuito.
  - q: ¿Por qué el gate de ensayo en M1?
    a: >-
      ...
      rentar cualquier GPU paga. Atrapa un typo de config, un bug de parser o
      una regresión en el pipeline de gráficos gratis, en vez de a $0.27/hr.
_source:
  hash: 7da3378307ad51d9
  locale: en
  translatedAt: '2026-05-29T02:19:23.613Z'
---
## Qué es
