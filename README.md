...
Keystatic is available during local development at `/keystatic`. It edits English project files and the English experience singleton only; after editing localized content sources there, run `pnpm translate` before committing. Keystatic serializes saved entries with its own YAML and MDX formatter, so review content diffs before committing.
